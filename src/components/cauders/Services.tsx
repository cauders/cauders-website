                        ...
                        <div key={service.slug} className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16 items-center">
                            <div className={cn("text-left", isEven ? 'md:order-1' : 'md:order-2')}>
                                <ScrollFadeIn direction={isEven ? 'left' : 'right'}>
                                    <h2 className="text-3xl font-bold text-foreground mb-4 font-headline">{service.title}</h2>
                                    ...
                                    <Link href={`/services/${service.slug}`}>
                                        <Card className="overflow-hidden group transition-all duration-300 hover:shadow-2xl hover:shadow-primary/20 hover:-translate-y-2 bg-card border text-left">
                                            ...
                                            <CardHeader className="p-6">
                                                <CardTitle className="mb-1 text-lg text-foreground font-headline">{service.caseStudy?.title}</CardTitle>
                                                <p className="text-sm text-foreground/70 line-clamp-2">{service.caseStudy?.description}</p>
                                            </CardHeader>
                                        </Card>
                                    </Link>
                                </ScrollFadeIn>
                ...
                <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
                    {whyChooseUs.map((reason, index) => (
                        <ScrollFadeIn key={reason.title} style={{ animationDelay: `${index * 150}ms`}} className="h-full">
                             <Card className="h-full text-center bg-card border hover:border-primary hover:-translate-y-2 transition-transform duration-300 group flex flex-col min-h-[300px]">
