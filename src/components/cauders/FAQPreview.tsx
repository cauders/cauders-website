...
export default function FAQPreview() {
    return (
        <div className="py-16 md:py-20 lg:py-32 relative overflow-visible">
                    ...
                    <ScrollFadeIn delay="delay-200">
                        <div className="max-w-3xl mx-auto">
                            <Accordion type="single" collapsible className="w-full space-y-4">
                                {faqs.map((faq, index) => (
                                    <AccordionItem key={index} value={`item-${index}`} className="bg-card rounded-2xl px-4 md:px-6 shadow-smooth-lift">
                                        <AccordionTrigger
                                            className="text-left text-xs md:text-base text-foreground font-medium hover:no-underline"
                                            icon={<Plus className="h-6 w-6 md:h-8 md:w-8 text-foreground" />}
                                        >
                                            {faq.question}
                                        </AccordionTrigger>
