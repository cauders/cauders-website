...
export default function PortfolioPreview() {
  ...
  const [carouselRadius, setCarouselRadius] = useState(-630);
  ...
  const calculatedRadius = carouselRadius / (2 * Math.tan(Math.PI / projects.length));

  useEffect(() => {
    const updateRadius = () => {
        if (window.innerWidth < 768) {
            setCarouselRadius(-500); // Smaller radius for mobile
        } else {
            setCarouselRadius(-630); // Default radius for desktop
        }
    };
  ...
  useEffect(() => {
    const animate = () => {
      ...
      if (carouselWrapRef.current) {
        ...
        carouselWrapRef.current.style.transform = `translateZ(${Math.abs(calculatedRadius)}px) rotateY(${currentRotateY.current}deg) rotateX(0deg)`;
      }
  ...
  return (
    ...
    >
      ...
      {/* 3D Carousel Area */}
      <div className="h-[35vh] md:h-[20vh] lg:h-[45vh] mt-8 md:mt-16 perspective-carousel pointer-events-none">
          ...
          {projects.map((project, index) => {
            ...
            return (
              <div
                ...
                className="absolute w-full md:w-[60vw] lg:w-[45vw] xl:w-[35vw] h-full top-0 left-0 right-0 mx-auto pointer-events-auto"
                onMouseEnter={() => setActiveCard(index)}
                onMouseLeave={() => setActiveCard(null)}
                style={{
                  transform: `rotateY(${cardRotation}deg) translateZ(${calculatedRadius}px)`,
                }}
              >
                <Link
                  ...
                  className={cn(
                    "block h-full w-full transition-all duration-500 ease-out rounded-2xl md:rounded-3xl group",
                    activeCard === index && "shadow-primary-glow"
                  )}
                  draggable={false}
                >
                  <Card className="h-full w-full overflow-hidden rounded-2xl md:rounded-3xl shadow-lg bg-transparent border-0">
                    <div className="w-full h-full relative overflow-hidden rounded-2xl md:rounded-3xl transition-transform duration-500 group-hover:scale-105">
                      <Image
                  ...
                  <div
                    className={cn(
                      "absolute w-[95%] bottom-0 p-3 md:p-6 lg:p-8 z-20 transition-all duration-700 ease-out text-left pointer-events-none",
                      "opacity-0 group-hover:opacity-100 group-hover:-translate-y-2 md:group-hover:-translate-y-4"
                    )}
                  >
                    <p className="text-sm md:text-base lg:text-xl font-garet text-white font-medium drop-shadow-md">
                      {project.description}
                    </p>
                    <h3 className="font-black text-base md:text-2xl lg:text-4xl text-white mt-1 font-headline drop-shadow-2xl">
                      {project.title}
                    </h3>
                    <p className="mt-1 md:mt-2 text-[10px] md:text-xs text-primary font-semibold uppercase tracking-wide drop-shadow-md">
                      {project.category}
                    </p>
                  </div>
                </Link>
