...
export default function Header() {
  ...
  const isHeroVisible = (pathname === '/' || pathname === '/careers') && !hasScrolled;
     ...
     <header className={cn(
      ...
      >
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-12">
          <div className="flex items-center justify-between h-14">
            <Link href="/" className={cn("text-xl md:text-2xl font-bold transition-colors text-foreground hover:text-primary", isHeroVisible ? "text-white hover:text-white/80" : "")}>
              Cauders
            </Link>
            
            <div className="flex items-center gap-4">
              <Button onClick={() => setIsMenuOpen(true)} variant="ghost" className={cn("text-base md:text-xl font-normal hover:bg-accent", isHeroVisible ? 'text-white hover:bg-white/10 hover:text-white' : 'text-foreground')} aria-label="Open menu">
                Menu
              </Button>
            </div>
          </div>
      ...
      >
        <Image
            ...
            fill
            className="object-cover absolute inset-0 z-0 blur-xl"
            priority
        />
                    ...
                    {navLinks.map((link, index) => (
                        ...
                        >
                            <MagneticLink
                                ...
                                className="w-28 h-28"
                                linkClassName="text-base text-background"
                            >
                                {link.label}
                            </MagneticLink>
                        </div>
